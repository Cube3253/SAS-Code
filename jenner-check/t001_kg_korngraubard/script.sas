...
* ------------------------------------------------------------------------- *;
* Caller for %calculate_KornGraubard_CIs (from KG_macro.sas).                  *;
* Builds a small mock SUDAAN Proc Descript output dataset with the variables  *;
* the macro requires (percent, sepercent, nsum, atlev1, atlev2) and invokes   *;
* the macro with its default options (df_adjust=yes, i.e. NHANES).            *;
* Values are illustrative only -- not real NHANES estimates.                  *;
* ------------------------------------------------------------------------- *;

data descript_out;
  length subgroup $12;
  input subgroup $ percent sepercent nsum atlev1 atlev2;
  datalines;
Male          31.4 2.1 812  15 30
Female        27.8 1.9 905  15 30
Age20_39      18.2 2.6 430  15 28
Age40_59      34.5 3.0 388  14 27
Age60plus     41.9 3.4 421  15 29
LowPrev        0.7 0.5 210  12 22
;
run;

%calculate_KornGraubard_CIs(dsIn = descript_out,
                            dsOut = descript_out_KG);

proc print data = descript_out_KG label noobs;
  var subgroup percent sepercent lowerCL upperCL pct_ci suppress statReview present;
run;
